                ...
                for (String[] psString : arrayList
                    ...
                    if (bkPrtQty - ioQuantity < 0) {//如果库存不足
                        ...
                        if (res == JOptionPane.YES_OPTION) {
                            String sql = "INSERT INTO OutOfStock(bkNo,oosQuantity,empNo,oosTime,oosUse,oosState) VALUES(?,?,?,?,?,?)";
                            //流水号	oosId 图书编号	bkNo 数量	oosQuantity 记录人编号	empNo 记录时间	oosTime 用途	oosUse
                            Date date = new Date();
                            DateFormat dateFormat = new SimpleDateFormat("yyy-MM-dd");
                            String df = dateFormat.format(date);
                            String[] ps = new String[]{bkNo, String.valueOf((ioQuantity - bkPrtQty)), LoginInfo.getLoginNo(), df, "订单出库缺货", "0"};
